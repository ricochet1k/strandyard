...
## Summary
There is no way to record incremental progress on tasks. Users can only mark entire tasks as completed via `memmd complete`, but cannot track progress on individual TODO items within tasks.
...
## Expected Result
A `memmd progress` command should exist that can:
...
## Acceptance Criteria
- `memmd progress <task-id>` shows current progress (X/Y completed)
- `memmd progress <task-id> --item <number>` marks specific TODO as complete
- `memmd progress <task-id> --item <number> --undo` marks TODO as incomplete
