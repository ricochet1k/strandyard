...
- **Alternative A**: Use "from now" for all metrics (consistent but ambiguous for git metrics).
- **Alternative B (Chosen)**: Use "from HEAD" for commit-based metrics (explicit and deterministic).
- **Alternative C**: Map "now" to "HEAD" in docs (preserves single concept but adds verbosity).
