...
1. Add a “Recurring tasks” section describing:
   - `strand recurring add` with required scheduling fields
   - `strand recurring materialize` for generating due tasks
...
- Run `strand repair` after doc updates if docs include embedded examples or references that affect templates.
