...
2. **Command shape**: add `strand issue add` (preferred for clarity) or `strand add --kind issue`. Use the existing task creation helper (if any) to avoid duplicated frontmatter logic.
