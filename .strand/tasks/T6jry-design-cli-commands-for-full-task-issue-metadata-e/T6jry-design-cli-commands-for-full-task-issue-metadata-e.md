...
- Current `strand add` command only supports limited flags (role, priority, parent, blockers)
...
- Option 3: Subcommand approach: `strand task add/edit`, `strand issue add/edit`
...
```bash
strand add leaf --interactive
strand edit ABC123 --interactive
```
...
```bash
strand task add leaf "Feature X" --role developer
strand task edit ABC123 --set-priority high
strand issue add "Bug description" --severity critical
strand issue edit I789 --add-blocker T123
```
...
**Cons**:
- Breaking change from current `strand add` usage
