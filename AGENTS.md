...
- **After commit**: Whenever you complete and commit a task, use the `session` tool with `mode: "new"` and `async: true` to start a new session with the exact text `do the next task`.
- **When blocked waiting on other work**: If the current task is blocked on reviews, owner decisions, or other tasks (including when you add wait-only subtasks), use the `session` tool with `mode: "new"` and `async: true` to start a new session with the exact text `do the next task`.
