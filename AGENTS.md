...
- **When blocked waiting on other work**: If the current task is blocked on reviews, owner decisions, or other tasks (including when you add wait-only subtasks), use the `session` tool with `mode: "new"` and `async: true` to start a new session with the exact text `do the next task`, then stop work in the current session.
